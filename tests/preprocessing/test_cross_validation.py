...
def test_get_train_test_splits_as_dataframe(test_dataframe: pd.DataFrame):
    ...
    my_ds = CircularCrossValidator(num_samples=len(test_dataframe), test_ratio=0.2, num_folds=50)
...
def test_circular_cv_with_sk_learn():
    ...
    my_cv = CircularCrossValidator(num_samples=len(iris.data), test_ratio=0.2, num_folds=50)
    ...
    assert (
        clf.n_splits_ == 50
    ), "50 folds should have been generated by the circular CV"
